...
export class GeoConvertController implements MergesRemoteSwagger {
	...
	async proxy(@Req() req: Request, @Res() res: Response, @Next() next: NextFunction) {
		...
		const { createdFileVersion, downloadType } =
			await this.triplestoreService.get<DownloadRequest>(downloadRequestId as string);
		if (["LIGHTWEIGHT", "AUTHORITIES_LIGHTWEIGHT"].includes(downloadType)) {
